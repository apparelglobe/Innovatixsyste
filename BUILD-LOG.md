...
### 2026-09-07 — Phase 4 · Slice 1: multi-project Relationship Home foundation — ✅ DEPLOYED
- **Built:** the portal moves from a single newest-project workspace to a relationship (multi-project) Home — **additive, no schema change.** New: `GET /portal/relationship-overview` (a COMPACT ProjectCard per project — no activities/team/reports/files — + ONE money-free Care Plan band + a server-authoritative `canBilling`); `GET /portal/projects` (a lean `id/name/status` list); the existing `GET /portal/projects/:id` is now wired into a real project-detail UI. `/portal/me` gains an additive `canBilling`. Web: a new **`useRelationship()`** hook powers the new Home/projects surfaces; the Home renders the aggregate cards + Care Plan band; a new `/projects/:projectId` detail route (approvals decided there); `/projects` is a project list; `PortalShell` gains a project **navigator** (switcher). **`/portal/overview` and `/portal/project` are UNCHANGED**; `/portal/project` (+ `usePortal`) is retained purely as **legacy compatibility** for the Billing / invoice-detail / Messages / Settings pages — its sole caller stays `usePortal.ts:43` (no new caller introduced).
- **Design guarantees:** the project navigator is **URL-driven** (`/projects/:projectId`), never persisted "current project" state, and is **hidden on Messages / Billing / Settings** (so it can't mislead where a message goes). **Billing isolation:** a card's `payableInvoice` is **OWNER-only AND project-linked only**; `projectId=null` / RETAINER / relationship-level invoices are **excluded from cards**; a **MEMBER gets `payableInvoice: null` on every card** (stable shape). **Care Plan is rendered once** at the relationship level, not per card. **Query strategy is batched (`WHERE projectId IN (...)`) → constant in N (no N+1).** **Deterministic ordering:** active (IN_PROGRESS/UAT/LAUNCHED/ON_HOLD) → planning (DISCOVERY) → completed (COMPLETE), then `updatedAt desc` / `id desc` within group.
- **Files:** `apps/api/src/portal/routes.ts`; web `app/page.tsx`, `app/projects/page.tsx`, `app/projects/[projectId]/page.tsx` (new), `components/PortalShell.tsx`, `lib/useRelationship.ts` (new). Test: `apps/api/test/integration/portal-multiproject.test.ts` (12). **No schema/migration/package/config change.**
- **Status:** ✅ **DEPLOYED to production 2026-09-07.** Branch `feat/slice1-relationship-home`, runtime checkpoint **`e16df967971833187069dd561f407d3cd3afa6c1`** (pushed). **API-first deployment order** (rsync `portal/routes.ts` → restart innovatix-api; backup `~/portal-routes.slice1.bak`), gated by an **API-first compatibility smoke while the old web was still live — 8/8 PASS** (legacy `/portal/overview` shape unchanged, `/portal/project` unchanged, `/portal/me.canBilling` OWNER=true/MEMBER=false, new endpoints work); **then** platform-web (`next build` → restart innovatix-portal). Local gate: portal-multiproject **12/12** (incl. constant query-count 1-vs-8), billing-authz **17/17**, client-rbac **16/16**, full regression clean. **Production smoke 21/21 PASS** (3-project OWNER/MEMBER: exactly-3 lean list, deterministic order, 3 cards, completed present, Care Plan once, OWNER project-linked payable on the correct card only, relationship-level invoice on no card, MEMBER `payableInvoice:null` on every card, `/portal/projects/:id` explicit routing, **cross-org project → 404**, **deactivated member → 401**, legacy shapes intact) + web routes serve (200); cleanup residue 0; **all 6 deployed files byte-identical to `e16df96`**; all services online, unstable restarts 0, no new 5xx. **Browser click-through was NOT directly exercised in production (environment limitation — no headless browser);** covered instead by deployed==tested source + `next build` + route-serving verification + the 21/21 API-data correctness. Deferred (Slice 2+): relationship activity feed (the per-project Home timeline is intentionally dropped until Slice 2), per-project messaging, full multi-project billing.

### 2026-09-07 — Phase 4 · Slice 0: client-session read authorization hardening — ✅ DEPLOYED
- **Built:** `requireSession` (client portal gate) previously authenticated only the JWT + tenant and never re-checked the caller's CURRENT DB-backed membership, so a **deactivated/removed** client kept portal READ access (overview / project / me / **file download** / notifications) until the 7-day token expired. Fix (centralized): `requireSession` now resolves the fresh role via `resolveClientRole` and returns **401** for a missing/inactive membership — one gate for all portal routes. It carries the resolved role in `ctx.role`, threaded into the 12 `requireClientPermission` calls so OWNER/write routes reuse that single lookup (1 membership query, not 2). `requireClientPermission` gained an optional `preResolvedRole` — supplied (incl. `null`) is trusted (a `null` denies, never bypasses); omitted → fresh DB resolve (backward-compatible). The JWT carries no role, so role is always the fresh DB value; **downgrade** takes effect immediately.
- **HTTP semantics (preserved):** invalid/expired/**inactive/removed** → 401; MEMBER → OWNER-only → 403; cross-org record → 404; cross-tenant session → 401.
- **Files:** `apps/api/src/portal/routes.ts`, `apps/api/src/client/authz.ts`. Test: `apps/api/test/integration/portal-session-authz.test.ts` (9). **No schema / migration / config / package / frontend change.**
- **Status:** ✅ **DEPLOYED to production 2026-09-07.** Branch `fix/portal-session-authz`, runtime checkpoint **`bb69534730b3494b46472532d049f1f8894c2d8b`** (pushed). **Code-only** — deploy = rsync `portal/routes.ts` + `client/authz.ts` + restart `innovatix-api` (backups `~/portal-routes.slice0.bak` + `~/client-authz.slice0.bak`; deployed hashes == committed). Local gate: portal-session-authz **9/9**, billing-authz **17/17**, client-rbac **16/16**, full regression clean (only pre-existing `/metrics` + Cal.com env fails). **Production smoke 17/17 PASS** (throwaway data, DB-verified, tokens never printed): active OWNER/MEMBER reads 200; **MEMBER billing-leakage check PASS** (OWNER sees `payableInvoice`, MEMBER `null`, MEMBER invoice detail 403); **authorized download PASS + byte-match, inactive-user download 401**; **inactive/removed client + existing token → 401**; **OWNER→MEMBER downgrade + existing token → OWNER-only 403**; reactivate + same token → 200; **cross-org project/file → 404**; **cross-tenant → 401**. Cleanup **residue zero**; no new 5xx; all services online. Query impact: +1 cheap PK lookup per authenticated read; OWNER/write routes stay at one lookup. First of the Phase-4 slices (Relationship Home follows in Slice 1).

### 2026-09-07 — Diagnosis: ERR_REQUIRE_ESM / @fastify/cookie — 🟢 NO CODE FIX REQUIRED (historical/stale log artifact)
- **Classification:** historical / stale PM2 error-log artifact — **not an active production bug.**
- **Original failing path:** `node dist/main.js` (the old pm2 CommonJS start command). **Current working path:** `tsx src/main.ts` (the pm2 runtime today; `tsx`'s loader interops ESM inside `require`).
- **Root cause:** a CJS→ESM boundary — `@fastify/cookie@11` depends on the **ESM-only `cookie@2.x`** (`cookie@2.0.1`, `type: module`); `@fastify/cookie/index.js` (CJS) does `require('cookie')`, which under pure-Node CommonJS (`node dist/main.js`) throws `ERR_REQUIRE_ESM` at module load → a boot-time crash-loop. Under `tsx src/main.ts` the same import loads cleanly.
- **Evidence:** `innovatix-api-error.log` last written **2026-07-18** with **32** `ERR_REQUIRE_ESM` entries whose stack traces originate from `apps/api/dist/main.js` via pm2 `require-in-the-middle` — **no new entries in ~7 weeks.** Controlled reproduction with the current dependency set (Node v18.20.8, fastify 5.10.0, @fastify/cookie 11.1.1): `node -e "require('@fastify/cookie')"` → `ERR_REQUIRE_ESM`; `tsx -e "import c from '@fastify/cookie'"` → `LOADED_OK`. Live: `/health` 200; cookie-dependent auth paths (`/v1/portal/overview`, `/v1/admin/leads`) → 401 (not 500); `@fastify/cookie` registers at boot (`main.ts:76`, awaited + uncaught, so reaching "Server listening" proves it); portal/staff session cookies (`reply.setCookie`) work; current out.log has 0 `ERR_REQUIRE_ESM`; unstable restarts 0.
- **Current production impact:** **none** — the running process boots cleanly, serves traffic, and cookie/session auth works.
- **Decision:** **leave production unchanged.** Do NOT change `@fastify/cookie`, downgrade/override `cookie`, touch the lockfile, change module settings, or alter the PM2/runtime startup. The stale error log is harmless historical evidence and may remain (not truncated for now).
- **Note:** reintroducing `node dist/main.js` as the start command would resurface this CJS→ESM incompatibility and **requires a fresh compatibility review** (e.g. pin/override `cookie` to a CJS-compatible version, or bundle the API) before use.
