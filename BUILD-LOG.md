...
### 🟢 RESOLVED (2026-09-06) — Browser file-download 503 was CLIENT-SIDE download-pipeline interception, not our origin
- **Verified fixed:** `s5-2-upload-test (12).pdf` downloads successfully and opens/renders from disk. Issue **closed**.
- **Root cause:** a **client-side** layer on the tester's machine hooking the browser's *download/attachment* pipeline (a download-scanning browser extension or endpoint security/DLP agent) synthesized a `503` **in the browser after the response left our origin**. Our server was never the cause.
- **Proven three ways (server never returned 503):** (1) the nginx access log shows every one of the tester's real navigate/iframe download requests to the exact URL returning **`200 668`** — zero non-200 for that URL, **zero `503`s in the whole access log**, error log clean; (2) raw HTTP with the exact `Sec-Fetch-Mode: navigate` / `Dest: document` / `Site: same-origin` headers returns **200** through nginx **and** direct — no server-side `Sec-Fetch` discrimination (`server: nginx/1.18.0`); (3) direct `fetch`/XHR = 200 while only Chrome's actual download machinery produced the 503. So the discriminator is the **client download path**, not any request the server sees.
- **Kept (not the trigger, but a correct hardening):** stored-file downloads now stream with a deterministic **`Content-Length`** (no chunked transfer-encoding) — deployed as commit `b73e5d462d5dbd8c3d3968067071c01bb8991860` (code-only, no migration; prod smoke 24/24). It was a reasonable low-risk hardening; it did NOT cause or fix the 503. **No further server/nginx change was needed or made.**
