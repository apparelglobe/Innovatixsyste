...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await api('/portal/auth/login', {
      method: 'POST',
    ...
    <main className="grid min-h-screen place-items-center bg-base px-6">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-line bg-surface p-6 shadow-card">
          <h1 className="text-lg font-bold text-white">Sign in to your portal</h1>
          <p className="mt-1 text-sm text-neutral-400">Access your project, reports, and approvals.</p>
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-neutral-200">Email</span>
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-neutral-200">Password</span>
              <input name="password" type="password" required autoComplete="current-password"
                className="w-full rounded-lg border border-line-strong bg-white/[0.03] px-3.5 py-2.5 text-sm text-white placeholder:text-neutral-500 focus:border-primary/60 focus:outline-none focus:ring-2 focus:ring-primary/30"
                placeholder="••••••••" />
            </label>
            {state === 'error' && (
              <p className="rounded-lg border border-red-500/30 bg-red-500/10 px-3.5 py-2.5 text-sm text-red-300">Invalid email or password.</p>
            )}
