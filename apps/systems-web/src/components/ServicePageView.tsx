...
export function ServicePageView({ page: p }: { page: ServicePage }) {
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden border-b border-line bg-base">
        ...
        <Container className="relative py-16 md:py-20">
          <nav aria-label="Breadcrumb" className="text-xs text-neutral-400">
            ...
            <a href="/services" className="hover:text-white">Services</a> <span className="text-neutral-600">/</span>{' '}
            <a href={categoryPath} className="hover:text-white">{p.category}</a>
          </nav>
          <div className="mt-4"><Badge>{p.category}</Badge></div>
          <h1 className="mt-4 max-w-4xl text-3xl font-extrabold leading-tight tracking-tight text-white md:text-5xl">{p.h1}</h1>
      ...
      <Section>
        <Container>
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Problems we solve</h2>
          <div className="mt-8 grid gap-5 md:grid-cols-3">
            {p.problems.map((pr) => (
              <Card key={pr.heading}>
                <h3 className="text-base font-bold text-white">{pr.heading}</h3>
      ...
      <Section tone="surface">
        <Container>
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">How we approach it</h2>
      ...
      <Section>
        <Container className="grid gap-12 lg:grid-cols-2">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">What you get</h2>
            <ul className="mt-6 space-y-3">
              {p.deliverables.map((d) => (
                <li key={d} className="flex items-start gap-2.5 text-sm text-neutral-300">
                  <Check size={18} className="mt-0.5 shrink-0 text-primary-light" /> {d}
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Technologies &amp; integrations</h2>
      ...
      <Section tone="surface">
        <Container>
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Our delivery process</h2>
          <ol className="mt-8 grid gap-6 md:grid-cols-5">
            {p.process.map((s, i) => (
              <li key={s.step}>
                <div className="text-3xl font-black text-primary/40">{String(i + 1).padStart(2, '0')}</div>
                <div className="mt-1 text-sm font-bold text-white">{s.step}</div>
        ...
        <Section>
          <Container>
            <Card className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-primary-light">Proof</div>
                <p className="mt-1 text-lg font-bold text-white">{p.proof.label}</p>
              </div>
      ...
      <Section>
        <Container className="max-w-3xl">
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Frequently asked questions</h2>
      ...
      <section className="relative overflow-hidden border-t border-line bg-base">
        ...
        <Container className="relative flex flex-col items-start justify-between gap-6 py-16 md:flex-row md:items-center">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Ready to scope your build?</h2>
