    ...
    <>
      ...
      <Section>
        <Container>
          ...
          <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-white md:text-3xl">Six phases, one connected system of record</h2>
            ...
            <ul className="space-y-3.5 text-sm text-neutral-200">
              {PORTAL.map((f) => (
                <li key={f} className="flex items-start gap-2.5">
                  <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-primary/15 text-primary-light">
                    <Check size={12} />
                  </span>
                  {f}
                </li>
      ...
      <section className="border-t border-line bg-base">
        <Container className="flex flex-col items-start justify-between gap-6 py-16 md:flex-row md:items-center">
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">See it applied to your project.</h2>
          <Button href="/book" size="lg">Book a Consultation <ArrowRight size={18} /></Button>
        </Container>
      </section>
    </>
