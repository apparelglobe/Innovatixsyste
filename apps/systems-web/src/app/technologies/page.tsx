    ...
    <>
      ...
      <Section>
        <Container>
          ...
          <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-white md:text-3xl">Proven tools, chosen for reliability</h2>
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {STACK.map((s) => (
              <Card key={s.area}>
                <h3 className="text-base font-bold text-white">{s.area}</h3>
                <p className="mt-2 text-sm leading-relaxed text-neutral-400">{s.body}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {s.items.map((it) => (
                    <span key={it} className="rounded-full border border-line bg-base px-2.5 py-1 text-xs text-neutral-300">
                      {it}
                    </span>
                  ))}
                </div>
