    ...
    <>
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-neutral-900 md:text-3xl">
                  <a href={`/services/${cat.key}`} className="hover:text-primary">{cat.label}</a>
                </h2>
                <p className="mt-2 max-w-2xl text-neutral-400">{cat.blurb}</p>
              </div>
              <a href={`/services/${cat.key}`} className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">
            ...
            <ul className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {cat.items.map((it) =>
                it.planned ? (
                  <li
                    key={it.href}
                    className="flex items-center justify-between gap-2 rounded-btn border border-neutral-200 bg-neutral-50/50 px-4 py-3 text-sm text-neutral-500"
                    title="Coming soon"
                  >
                    {it.label}
                    <span className="rounded-pill border border-neutral-200 px-2 py-0.5 text-[10px] uppercase tracking-wide text-neutral-500">Soon</span>
                  </li>
                ) : (
                  <li key={it.href}>
                    <a
                      href={it.href}
                      className="group flex items-center justify-between gap-2 rounded-btn border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm font-medium text-neutral-700 transition hover:border-primary/40 hover:text-neutral-900"
                    >
                      {it.label}
                      <ArrowUpRight size={15} className="text-primary opacity-0 transition group-hover:opacity-100" />
                    </a>
                  </li>
                ),
              )}
      ...
      <section className="relative overflow-hidden border-t border-neutral-200 bg-white">
        ...
        <Container className="relative flex flex-col items-start justify-between gap-6 py-16 md:flex-row md:items-center">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-neutral-900 md:text-3xl">Not sure where to start?</h2>
            <p className="mt-2 text-neutral-600">Tell us the problem — we&apos;ll map the fastest path to a working system.</p>
          </div>
          <Button href="/book" size="lg">Book a Consultation <ArrowRight size={18} /></Button>
        </Container>
      </section>
